...
This is a tutorial to make a Shiny app with the R golem package: longer than [the official cheat sheet](https://thinkr.fr/golem_cheatsheet_V0.1.pdf) and shorter than [Engineering Production-Grade Shiny Apps](https://engineering-shiny.org/).

## Setup a golem project
...
Install the golem package on R.

```{r install_golem, eval=FALSE}
install.packages("golem")
```

Create a golem project on RStudio in the following steps.
...
Some files may be ever unused and we can remove them from the repository before you release the project.
...
Second, we write back-end logic scripts in test-driven development. In this project, __inst/nbinomPlot.Rmd__ describes how the back-end of nbinomPlot works.
...
`R CMD check` tells us that explanation of some functions is missing. It is a way to make an R package to build perfect components one by one rather than write comments after writing all code. We should keep no errors and no warnings in developing an R package in any stage.

R Markdown documents are useful to explain back-end logic with images and running code. Now we edit __inst/nbinomPlot.Rmd__, convert it to an HTML document, and check the HTML document. In RStudio, we can convert an R Markdown document with one click.
...
To publish readable code, a code formatter comes in handy and the styler R package formats R code pretty. Note that code formatters do not change behaviors of code but it is still needed for us to compare and check the code is virtually unchanged before and after format it.
