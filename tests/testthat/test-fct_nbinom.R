...
test_that("calculate_nbinom_density_fine", {
  tolerance <- 1e-7
  expected <- c(
    0.31640625, 0.34606934, 0.31640625, 0.25955200, 0.19775391, 0.14275360,
...
test_that("get_nbinom_density_dataframe_int", {
  ...
  df_actual <- get_nbinom_density_dataframe(
    size = 8, prob = 0.85,
...
test_that("get_nbinom_density_dataframe_fine", {
  tolerance <- 1e-7
  expected <- c(
    0.272490525, 0.336014682, 0.281860722, 0.189832909, 0.110358663,
...
test_that("draw_nbinom_density", {
  expect_s3_class(draw_nbinom_density(
    size = 4.0, prob = 0.25,
    lower_quantile = 0.999, step = 1.0
  ), "gg")
})
